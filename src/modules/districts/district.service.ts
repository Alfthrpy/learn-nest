...
export class DistrictService {
  ...
  async createFromGeoJson(geoJsonValue: string, layerId: number): Promise<any> {
    if (!layerId) {
      throw new BadRequestException('Layer ID is required');
    }

    const feature = await this.prisma.$queryRaw<any[]>(Prisma.sql`
        INSERT INTO "features" ("layer_id", "geom","updated_at")
        VALUES (${layerId}, ST_SetSRID(ST_GeomFromGeoJSON(${geoJsonValue}), 4326), NOW())
        RETURNING *
      `);
